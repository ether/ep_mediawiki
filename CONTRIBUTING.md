# Contributing to ep_mediawiki

Thanks for helping improve `ep_mediawiki` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_mediawiki` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_mediawiki from your local clone
pnpm run plugins i --path ../ep_mediawiki

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_mediawiki/` are picked up after a server restart.
...
* Etherpad version, ep_mediawiki version, browser + OS
* Steps to reproduce
